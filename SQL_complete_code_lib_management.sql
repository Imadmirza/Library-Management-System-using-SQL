...
SELECT ist.issued_member_id AS member_id, 
       ...
       SUM((DATEDIFF(CURDATE(), ist.issued_date) - 30) * 0.50) AS total_fines
FROM issued_status AS ist
...
SELECT 
    ...
    COUNT(*) AS overdue_books,
    SUM((DATEDIFF(CURDATE(), ist.issued_date) - 30) * 0.50) AS total_fines,
    (SELECT COUNT(*) FROM issued_status WHERE issued_member_id = ist.issued_member_id) AS total_issued_books
FROM issued_status AS ist
LEFT JOIN return_status AS rs ON ist.issued_id = rs.issued_id
WHERE rs.return_id IS NULL 
AND DATEDIFF(CURDATE(), ist.issued_date) > 30
